            ...
            if self.target == nil then
                ...
                if self.turnCount >= 10 then
                    ...
                    for k,v in pairs(ents.FindInSphere(self:GetPos(), self.radius)) do 	 	--looking for prey
                        ...
                        if ((!self.ignore[class] and v != self and (v:IsNextBot() or (!v:IsPlayer() or GetConVarNumber("ai_ignoreplayers") == 0) or v:IsNPC()) and v:Health() > 0) or (v:IsVehicle() and self.vehicles[v:GetVehicleClass()] and v:GetDriver() != NULL)) and v:WaterLevel() > 0 and self:WaterLevel() > 0 then
                ...
                for k,v in pairs(ents.FindInSphere(self:GetPos(), self.radius)) do 	 --check if the prey still here	    
                    if v == self.target and IsValid(v) and (((v:IsNextBot() or (!v:IsPlayer() or GetConVarNumber("ai_ignoreplayers") == 0) or v:IsNPC()) and v:Health() > 0) or v:IsVehicle()) and v:WaterLevel() > 0 and self:WaterLevel() > 0  then
                        target = true
                        break
                    end 
                end
